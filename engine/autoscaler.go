...
func (a *Autoscaler) getQueueInfo(_ context.Context) (runningTasks, pendingTasks int, err error) {
	...
	agentLabels := a.config.ExtraAgentLabels
	requiredLabels := make(map[string]string)
	for label, value := range agentLabels {
		if strings.HasPrefix(label, "!") {
			requiredLabels[label[1:]] = value
		}
	}

	running := countTasksByLabels(queueInfo.Running, requiredLabels)
	pending := countTasksByLabels(queueInfo.Pending, requiredLabels)
...
func (a *Autoscaler) calcAgents(ctx context.Context) (float64, error) {
	...
	log.Debug().Msgf("capacity info: reqAgents = %v pool = %v/%v limits = %v/%v", reqAgents, availablePoolAgents, reqPoolAgents, maxUp, maxDown)
